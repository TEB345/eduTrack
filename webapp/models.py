...
class Attendance(models.Model):
    ...
    date = models.DateField(auto_now_add=True)  # Separate date field
    time = models.TimeField(auto_now_add=True)  # Separate time field
    status = models.CharField(max_length=1, choices=ATTENDANCE_CHOICES, default='P')
    method = models.CharField(max_length=10, default='AUTO')
    verified = models.BooleanField(default=False)

    class Meta:
        unique_together = ('studentNumber', 'subjectCode', 'date')

    def __str__(self):
        return f"{self.studentNumber} - {self.subjectCode} ({self.status})"
